...
// Helper para executar um único tool call (reutilizado em ambos os modos)
const executeToolCall = async (toolCall, allUserCustomTools) => {
  ...
  try {
    ...
    const errorMessage = "Argumentos JSON inválidos fornecidos pela IA."
    console.error(`[TOOL_EXECUTION_ERROR] Falha ao parsear argumentos para ${functionName}:`, e)
    return { success: false, name: functionName, content: JSON.stringify({ error: errorMessage }), message: errorMessage }
  }
  ...
  try {
    ...
    return { success: true, name: functionName, content: functionResponseContent }
...
const sendMessage = async (req, res) => {
  try {
    ...
    if (stream) {
      ...
      const streamResponse = await ask(aiProvider, aiKey, messages, { ...requestOptions, stream: true })
          ...
          delta.tool_calls.forEach(toolCallChunk => {
            ...
            if (!existingCall) {
              ...
              if (toolCallChunk.function?.arguments) {
                aggregatedToolCalls[toolCallChunk.index].arguments += toolCallChunk.function.arguments
              }
            }
          })
        }
      ...
      for (const toolCall of finalToolCalls) {
        ...
        const result = await executeToolCall(toolCall, allUserCustomTools)
        const status = result.success ? "SUCCESS" : "FAILURE"
        writeStreamData(res, { type: "status", status: "TOOL_EXECUTION", tool: { name: result.name, state: status, message: result.message } })
      ...
      const { data } = await ask(aiProvider, aiKey, messages, { ...requestOptions, stream: false })
      const responseMessage = data.choices[0].message

      if (!responseMessage.tool_calls) {
        return res.status(200).json({ finalMessage: responseMessage, executionLog })
      }
      ...
      for (const toolCall of responseMessage.tool_calls) {
        executionLog.push({ type: "status", status: "TOOL_CALL", tool: { name: toolCall.function.name, state: "CALLING" } })
        const result = await executeToolCall(toolCall, allUserCustomTools)
        const status = result.success ? "SUCCESS" : "FAILURE"
        executionLog.push({ type: "status", status: "TOOL_EXECUTION", tool: { name: result.name, state: status, message: result.message } })
        messages.push({
      ...
      executionLog.push({ type: "status", status: "TOOL_PROCESSING", tool: null })

      const finalResponse = await ask(aiProvider, aiKey, sanitizeMessages(messages), { ...requestOptions, stream: false })

      executionLog.push({ type: "status", status: "TOOL_PROCESSING_COMPLETE", tool: null })

      return res.status(200).json({
        finalMessage: finalResponse.data.choices[0].message,
        executionLog
      })
    }
